...
- **Code Scale**: 318 个 .cs 文件，约 108K 行代码，56 个 AgentTool
- **Status**: Phase 1~6 已验收；治理层 G.1~G.3 完成；Phase 7~9 完成；v1.7.x UX/工具精简冲刺；v1.8.0 能力覆盖面 P0 补齐（Profiler 三件套 + G10/G17/G18）；v1.9.0 L10n 多语言；v1.10.0~v1.10.6 Windows 稳定性 hardening（21 bugs 全修）；v1.12.0 Session Organization；v1.13.0 Provider Profile 系统 + Error-driven Request Pruning（多 Provider 管理/一键切换，自动学习 400 禁字段，EditorPrefs 持久化，tool_call.id 跨供应商清洗）
