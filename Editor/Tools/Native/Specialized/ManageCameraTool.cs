...
namespace AgentCore.Editor.Tools.Native.Specialized
{
    ...
    [AgentTool("manage_camera",
        Description = "Unity Camera component management — create, configure, inspect, align, and render, plus SceneView camera control (v1.9.7 G09 + orbit/pan/dolly/frame). " +
                      ...
                      "5) pan_scene_view to slide the view sideways when the target is off-center, 6) OBSERVE LOOP (pair with vision_analyze): close the loop by observing the view you just set — call vision_analyze (source=scene, action=analyze with a prompt for the user's visual question); if its description shows the target is still unclear/out of frame/occluded/too small, go back to steps 3-5 to re-navigate (orbit/pan/dolly) then vision_analyze again — repeat until you can answer, then stop (after ~3-4 repositions without convergence, report and ask). vision_analyze source=scene observes the SAME SceneView.lastActiveSceneView these navigation actions drive, so they compose into one loop. NOTE: the vision model only describes a fixed frame — it does NOT propose camera moves; YOU decide the next viewpoint. Requires activating BOTH categories [Specialized] (manage_camera) and [Meta] (vision_analyze) via request_tools. " +
    ...
    public class ManageCameraTool : IAgentTool
    {
        ...
        public ToolMetadata Metadata => new ToolMetadata(
            ...
            description: "Create, configure, inspect, align, and render Unity Cameras, plus SceneView camera navigation (orbit/pan/dolly/frame). " +
                         ...
                         "4) dolly_scene_view (factor 0.5 to zoom in for detail, 2 to zoom out for overview), 5) pan_scene_view (dx/dy) to slide, 6) OBSERVE LOOP (pair with vision_analyze): confirm/answer the visual question by observing the view you set — vision_analyze (source=scene, action=analyze); if its description shows the target is still unclear/out of frame/occluded/too small, re-navigate (steps 3-5) and vision_analyze again — repeat until you can answer, then stop (after ~3-4 repositions without convergence, report and ask). vision_analyze source=scene observes the same SceneView this drives. The vision model only describes the fixed frame — it does NOT propose camera moves; you decide the next viewpoint. Needs BOTH categories activated: [Specialized] and [Meta]. Prefer relative increments over absolute transforms.",
            category: "Specialized",
