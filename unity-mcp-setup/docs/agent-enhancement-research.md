...
> ⚠️ This solution has not been validated in practice; specific implementation approach may be adjusted.
...
| **Semantic Memory** | General knowledge | User preferences, domain knowledge | ⚠️ Only architecture-snapshot exists |
...
| 2 | **MCP** | ⚠️ Unity MCP v9.5.3 configured | 🟡 Small gap | P2 |
| 3 | **Hooks** | ❌ Completely missing | 🔴 Large gap | See decision evolution |
| 4 | **Memory Systems** | ❌ Completely missing | 🔴 Large gap | **P1** |
| 5 | **Context / Project Memory** | ⚠️ project-overview is an empty template | 🟡 Medium gap | P2 |
| 6 | **RAG / Knowledge Base** | ❌ Completely missing (not needed at current scale) | 🟠 Medium gap | P3 |
| 7 | **Multi-Agent / Subagents** | ⚠️ Roo Code multi-mode available | 🟡 Small gap | P4 |
...
| **GitHub Copilot** | ✅ Native (requires enabling) | ✅ | ✅ | ⚠️ Preview | VS Code requires setting `chat.useAgentsMdFile: true` |
...
| **Cursor** | ⚠️ Needs workaround | ⚠️ MDC format | ✅ | ✅ 15+ events | Reference AGENTS.md in `.cursorrules` |
| **Continue** | ⚠️ Config-driven | ⚠️ | ✅ | ❌ | Requires configuration in config.json |
...
| **Long-task degradation** | Behavior in the later stages of complex tasks is increasingly driven by recent context | ⚠️ Limited help |
...
> ⚠️ The following is an unvalidated design draft, targeting a 150-person team (single-person module ownership model).
