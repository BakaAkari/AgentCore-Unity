...
> **⚠️ Version Compatibility Warning (2026-03-26 verified through testing)**
>
...
> ⚠️ **Do not use `#main`**: The `main` branch is equivalent to v9.6.2 and will fail to compile on Unity 2022.3 and below due to the missing `BuildReport.SummarizeErrors()` API. See the version compatibility warning at the top of this document.
...
"@
    ...
    if ($Force -or -not (Test-Path $claudeDesktopPath)) {
        ...
        Write-Host "  ⏭️  Claude Desktop: Already exists, skipped (-Force to overwrite)" -ForegroundColor DarkYellow
    }
    ...
    if ($Force -or -not (Test-Path $cursorPath)) {
        ...
        Write-Host "  ⏭️  Cursor: Already exists, skipped (-Force to overwrite)" -ForegroundColor DarkYellow
    }
    ...
    if ($Force -or -not (Test-Path $vscodePath)) {
        ...
        Write-Host "  ⏭️  VS Code: Already exists, skipped (-Force to overwrite)" -ForegroundColor DarkYellow
    }
...
if ($script:AllPassed) {
    ...
    Write-Host "  ⚠️  Some checks failed, please fix according to the prompts" -ForegroundColor Yellow
}
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
