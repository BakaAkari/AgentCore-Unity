...
| **模式系统** | ✅ (Plan/Act) | ✅ (Code/Architect/Ask/Debug/Custom) | ✅ (自主度滑块) | ❌ | ⭐⭐⭐ 高 |
...
| **无上下文压缩** | 长对话后性能下降 | ⭐⭐⭐ 高优先级 |
| **无模式系统** | 所有任务使用相同的 System Prompt | ⭐⭐⭐ 高优先级 |
...
Phase 6.1 — 上下文压缩与模式系统 (v0.5.1)
  ├─ 6.1.1 上下文摘要策略
  ├─ 6.1.2 Architect Mode（规划模式）
  ├─ 6.1.3 Review Mode（审查模式）
  └─ 6.1.4 模式切换 UI
...
| 上下文压缩导致信息丢失 | 中 | 中 | 保留最近 N 轮完整对话，只压缩旧消息 |
| 模式系统增加用户学习成本 | 低 | 低 | 默认使用 Chat Mode，高级用户可选择其他模式 |
| 规则系统被滥用（规则过于复杂） | 低 | 低 | 提供规则模板和最佳实践文档 |
...
```
AgentCore = Unity 深度集成 + 代码库理解 + 工具生态 + 模式系统
```
...
1. **立即开始**: 代码库索引 MVP（v0.5.0）
2. **并行设计**: Architect Mode 和规则系统
3. **持续迭代**: 根据用户反馈调整优先级
