...
> **版本**: v1.16.0 | **更新日期**: 2026-08-19 | **状态**:
> Phase 6 验收完成（v1.0.0）；治理层 G.1~G.3 全面完成（v1.1.0）；Phase 7 §3.1 后台增量索引 + §3.2 ThinkingDrawer 可观测性完成（v1.2.0）；Request Enrichment 修复 reasoning 触发（v1.2.1）；v1.3.x 系列稳定性修复；v1.4.0 索引 Scope 层次化；v1.4.1~v1.4.9 VCS 组件修复链 + Phase 9 骨架；v1.5.0-alpha1/2 Phase 9 Self-Challenge 核心 + ADR-17 极简哲学；v1.5.0-alpha4~alpha5 model-tier escape + GLM-5.2 适配；v1.5.6~v1.5.7 稳定性修复；**v1.6.x 系列产品化体验冲刺**（Context Ingest / YOLO 信任模式 / 日志分级 / PendingIndicator / SSE yield 优化 / 消息引用栏 / Play Mode preflight / 多轮思考窗口 / 文件删除视觉反馈 / GLM-5.2 reasoning 参数适配 / 自适应 LLM 配置 / 统一 LLM 管道 / 气泡溢出修复 / 流式 UI 性能优化）；**v1.7.0 Settings v20 死字段清理 + VCS 模块修复**（12 字段删除 / 进程句柄泄漏 / 事件泄漏 / Debug.Log 风暴 / 设置极简化 8→2 / Project 窗口多 VCS 支持）；**v1.7.1 新装路径 + VCS 开机触发 + CS0162 修复**；**v1.7.2 文档整理归档（无代码变化）**；**v1.7.3 老项目升级安装 Preferences 目录 Move 弹窗修复（beforeAssemblyReload 回调）**；**Phase 9 Self-Challenge 收尾**（GLM-5.2 替代 Qwen 后 LLM 自身能力填上缺口）；**v1.7.x 系列 UX/工具通用能力冲刺**（ask_user 中途提问 / 工具确认流程重构 / execute_code 完全重写 / 工具精简五连弹 / Undo 覆盖补齐）；**v1.8.0 能力覆盖面补齐 P0**（Profiler 三件套 + G10/G17/G18）；**v1.9.0 L10n + 治理层 G.1~G.3**；**v1.10.0~v1.10.6 Windows 稳定性 hardening**（21 bugs 全部修复或驳回）；**v1.12.0 Session Organization**（tag / 归档区 / 自动命名 debounce / 移除 Silent mode）；**v1.13.0 Provider Profile 系统 + Error-driven Request Pruning**（多 Provider 配置管理一键切换 / error-driven 字段裁剪自学习 / EditorPrefs 持久化 / tool_call.id 跨供应商清洗）；**v1.14.0 Playmode 写权限白名单反转 + manage_component:add 根因修复**（fail-closed：write action 默认硬禁止仅显式登记放行 / MarkSceneDirty Playmode 异常真正根因修正 / README 开源化重写 + LICENSE 改 MIT）；**v1.14.1 默认 Provider Profile 切换内网 NewAPI 网关 + Quick Actions 按钮文案修复**（首次自动创建的 Default profile 从内网 GLM endpoint 切到 NewAPI 网关，无用量限制可统计用量 / Dashboard "Clear Learned Request Rules" 按钮文字截断修复）；**v1.14.2 修复新项目安装后直接打开 Chat Window 初始化失败**（Default profile 自动创建逻辑从 UI 层下沉至数据层单一入口 `AgentCoreProviderProfiles.EnsureDefaultProfileIfEmpty`，消除对 Settings 面板打开顺序的隐性依赖 / Default profile 名称去掉括号）；**v1.14.3 修复 v1.14.2 遗留缺口——首次对话必然 HTTP 400**（自动创建的 Default profile modelName 留空，AgentLoop 初始化路径缺少等价的自动 fetch 模型名逻辑；新增 `EnsureDefaultProfileWithAutoModelAsync` 并改为 await，加 8s 超时保护不阻塞初始化）；**v1.14.4 修复长任务思考链渲染撞 UIToolkit 顶点上限 + prompt 层任务拆分纪律**（`ThinkingDrawer` 全量文本塞入单 Label 撞 65535/49152 顶点上限，改为 6000 字滚动渲染窗口，完整内容仍持久化不裁剪；`AppendReasoning` 改用 StringBuilder 累积消除 O(n) 拼接性能债；`SOUL.md` 新增规则 2b：明确方案后 4+ 步骤任务先输出可见分步计划再逐步执行汇报，与 Node A 意图澄清互补不重复）
...
| 维度 | 状态 |
|------|------|
| **版本** | **1.16.0 (2026-08-19)** — v1.16.0: 用户聊天上传图像支持（`+` 附件按钮 → `UserImageStore` → `vision_analyze source=user_image` 读取分析闭环）+ agent 主动向聊天发送截图（新增 `send_image` 工具：`source=scene/game/user_image` 捕获/取图 → assistant 气泡渲染并持久化到 `turn.ImageDataUrl`）+ 输入区右下角布局重排（`◼` 方块取消按钮、`+`/思考强度 auto 下拉与发送按钮中心对齐）；**1.15.4 (2026-08-19)** — v1.15.4: 会话 tag 排序交互优化（右上排序按钮：按名称/修改/创建时间排序；右键菜单去除上移/下移、仅保留置顶，置顶 tag 组头显示 ▲ 标记；归档区 tag 标识左对齐）+ 视觉观察闭环（observe loop：manage_camera 导航与 vision_analyze 观察同一 SceneView 组成闭环）引导层升级；**1.14.13 (2026-08-11)** — v1.14.x: reasoning 强度会话级快捷切换（`ReasoningParamMapper`，UI 层抽象等级 ↔ 供应商特征分组映射，不按模型名特判）+ 跨会话全文检索 `search_sessions`（硬顶限流）+ DeepSeek-V4-Flash（vLLM 0.25.1）reasoning 内容混入正文修复（`chat_template_kwargs.thinking=true`）+ Self-Challenge 标签外复述泄漏修复（`PromptEchoScrubber`）+ 流式分块双阈值 flush + 多轮旁白 vs 结论分界（`ConversationTurn.RoundContents`）+ 假完成单次拦截、连续静默诚实呈现 + ThinkingDrawer UI 顶点满修复（6000 渲染窗口）、流式截断/切回完整根因修复 + 流式 tool_call 缺 id → 400 修复（`ToolCallBuilder.Build()` Guid 兜底）+ DeepSeek-V4 家族误判非 native-reasoning 致双重思考修复（`deepseek-r`→`deepseek` 逃逸 Node A）；v1.14.0 Playmode 写权限白名单反转（fail-closed）；v1.13.0 GA：Provider Profile 系统（`ActiveModelConfig` 唯一入口 / error-driven Request Pruning / `RequestPruningRegistry` EditorPrefs 持久化 / `tool_call.id` 跨供应商清洗）；v1.12.x Session Organization（tag registry / 归档区 / 流式 SSE `EndOfStream` 阻塞根因修复 / Silent 模式移除）；v1.10.x~1.10.6 Windows hardening；v1.9.0 多语言 L10n；v1.8.0 能力覆盖 P0 补齐；v1.7.x UX 冲刺。完整演进历史见 `CHANGELOG.md` |
| **代码规模** | 约 318+ 个 .cs 文件，约 108K+ 行代码，**约 60 个 AgentTool**（2026-08 实测：59 个工具定义文件 / 60 个 AgentTool 实例） | 对抗式校验发现的 R1+R3 SOUL 引导层缺陷治理。用户第 6 轮追问"从一致性原理再看系统缺哪些 Unity 工具模块覆盖"触发我做对抗式校验:枚举 47 个原生工具×~35 个 Unity Editor 常用模块得覆盖率 ~85% + 余 5 模块 execute_code 全兜底,R2(action 型 dispatcher default case 参差)v1.7.14~v1.7.25 五连弹已扫至 44/44 全绿。对抗式发现真实系统性缺陷两处仅在引导层:R1 跨调用资源时序陷阱(创建 asset→立即用 path 未 SaveAssets+Refresh,新 .cs 类型反射需 Domain Reload 无法单块内完成),SOUL.md 完全没提 AssetDatabase.Refresh/delayCall/SaveAsPrefabAsset;R3 execute_code 修改 GameObject/Component 忘 Undo.RecordObject 致用户 Ctrl+Z 无效——静默的能力残缺不报错但违反 Editor 契约。修复(SOUL §2.10 第 10 条内追加,不新增条款编号维持 1~12 稳定):Undo 契约(任何 scene 对象修改前置 Undo.RecordObject,new GameObject 后立即 Undo.RegisterCreatedObjectUndo,批量走 SetCurrentGroupName+CollapseUndoOperations,跳过 Undo 视为破坏输出契约而非小瑕疵);跨调用时序(AssetDatabase.SaveAssets+Refresh 必须同 call,新脚本类型反射拆分工作流→告知用户等重编译或 EditorUtility.RequestScriptReload,prefab 用 PrefabUtility.SaveAsPrefabAsset 不自造 File.WriteAllText)。文件仅改 SOUL.md/package.json/README.md/plans/ROADMAP.md/CHANGELOG.md 五处,零代码变更纯引导层升级。v1.8.0 时机建议:锚定 Phase 8 MCP Server 对外可交付首版而非累积 patch 数量,v1.7.x 继续做内部完善收尾直到 Phase 8/自演化知识系统落地才 bump minor；1.7.27 (2026-07-22) — v1.7.27: execute_code 5 轮反复失败根因治理（不是程序集问题,是错误分类器 bug）。v1.7.26 首次用 StreamReportPrinter 拿完整诊断 + error/warning 分离,但 safe-default 分支把 CS1685 warning 续行 "(Location of the symbol related to previous warning)" 误判为 error, 导致所有求值 FAIL。前 5 轮盲改 ReferenceAssembly 列表(CS0433/CS0246/CS0234 每次换新错误)全走偏, 从未质疑分类器。用户第 5 轮暴怒后换 Claude Opus 4-7 全量分析 554 行源码定位分类逻辑。核心修复:翻转 safe-default——严格正则 `\berror\s+CS\d+\b` 白名单归 error,续行/未知格式一律归 warning, 以后 Mono.CSharp 换版本输出未知诊断也不误判。搭车恢复 v1.7.0 全部 10 usings (System/System.IO/System.Text/System.Text.RegularExpressions/System.Linq/System.Collections.Generic/UnityEngine/UnityEngine.SceneManagement/UnityEditor/UnityEditor.SceneManagement)——v1.7.26 因误判缩到 4 个作"最小实证配置", 分类器修好后表象自动消失。新增 UnityEditor.SceneManagement 程序集引用(EditorSceneManager 在独立 UnityEditor.SceneManagerModule.dll),HashSet 去重防同 dll 二次引用产生 CS0433。双通道 Debug.Log 去重:v1.7.26 同时订阅 logMessageReceived + logMessageReceivedThreaded 致 Editor 主线程日志出现两条,改为只订阅 Threaded(覆盖所有线程含主线程,是完整覆盖)。Evaluate 剩余字符串(尾部无法解析,未闭合括号等)归入 compileError 而非 runtimeError——本质是编译期失败,契约一致。三 case smoke test 全 PASS(A:var x=40;x+2 返回 42/B:Debug.Log("SMOKE_B") output 单条不重复/C:__result="legacy" 报 CS0103)。静态验证:{} 58/58、() 198/198、[] 44/44,logMessageReceived 无 Threaded 变体归零,'\r','\n' char 字面量正确。搭车杂项:AgentCoreSettings v20→v21 迁移补执行 disabledTools 里 execute_code 单项移除(v20 声称清理但实际没执行 Clear 致 ToolRegistry 过滤掉 execute_code 令 SOUL §2.10 引导的能力不可用),不清空整个 disabledTools 尊重用户其它禁用意图; LightRAGClient query 超时 30s→180s(PostAsync 新增可选 timeoutSeconds 参数,query 走 180s,索引/文档管理/健康检查保持 30s), 冷启动/刚索引完首次查询实测持续超时; Mem0Client add 失败识别 user_not_found(404 "User not found")自动调 CreateUserAsync 隐式注册后重试一次(allowAutoRegister 防无限递归),让记忆功能开箱即用；1.7.26 (2026-07-22) — v1.7.26: execute_code 工具完全重写。根因治理:v1.7.0 起反射 new Evaluator(CompilerContext) 构造的评估器没有 InteractiveBase REPL 全局作用域绑定,对未声明变量做 __result = value 赋值 C# 编译器视为编译错误(不是 REPL 语义);Mono.CSharp 的 ConsoleReportPrinter 把错误写到进程 Console.Error 既不进 Unity Console 也不进 Application.logMessageReceived,被工具静默吞掉。结果失败被伪装成成功:agent 拿到 success:true / result:null / resultType:void,连 Debug.Log 输出也 0 条,连续 5 次撞墙无解。彻底改法(不修补):(1) __result 约定完全废除,返回值语义改为 Roslyn/IPython 风格 —— 代码块最后一项是表达式(无 ; 结尾)则返回其值,是语句则不返回。示例:"var x = 40; x + 2" 返回 42;"Debug.Log("ok");" 不返回(正常成功)。(2) action 参数删除,execute_code 只有一个入口(code + 可选 context),旧 evaluate 分支完全删除(~-300 行:HandleEvaluate/EvaluateExpression/ResolveType/IsAllowedType/ParseMethodArguments/SplitArguments/ConvertArgument/AllowedNamespaces)。action="run" 兼容一版本;其他值直接失败。(3) 编译错误从静默吞掉改为失败并附错:Console.SetError(new StringWriter()) 重定向 stderr 拿回 Mono.CSharp 完整诊断,作为 data.compileError 返回 + ToolResponse.Fail 而不是 OkWithData —— 再也没有 "success:true / result:null / 没日志" 的黑盒。(4) Run+Evaluate("__result") 两段式改为单次 Evaluate(string, out object, out bool),Mono.CSharp 3 参 Evaluate 本身吃多语句块+末尾表达式(csharp REPL 命令行的运作方式),一次调用同时得到 result 和 result_set。(5) Debug.Log 捕获保留(Application.logMessageReceived → data.output)。(6) SOUL §2.10 同步重写,给三个示例,明确"legacy __result pattern removed in v1.7.26"。文件 ExecuteCodeTool.cs 764→~430 行。静态验证:{} 57/57、() 179/179,legacy 标识符残留 0。**版本 bump 到 1.7.26 而非 2.0**:execute_code 是 Visibility=Restricted、仅 agent 通过 SOUL 使用、无外部脚本 API 依赖,从用户视角是 agent 行为改善;若未来加 MCP 暴露再考虑 2.0。；1.7.25 (2026-07-21) — v1.7.25: 工具通用能力升级第 5 弹(smart_operations 大瘦身,收官)。SmartOperationsTool 从 7 action 精简到 1。删除的 6 个 action 全部可用 execute_code:run 一段 3-8 行 C# 完整覆盖:align_objects(LINQ Min/Max/Average + foreach 赋值)、distribute_objects(等距间隔 + foreach)、snap_to_grid(Mathf.Round(x/g)*g 三行)、align_to_ground(Physics.Raycast Vector3.down 五行)、randomize_transform(UnityEngine.Random.Range 八行);其中 select_by_criteria 是与 find_gameobjects 工具的 duplicate 功能(searchTerm/tag/layer/componentType/activeOnly 五维过滤已覆盖 name_contains/tag/layer/component 四维,static_only 用 execute_code .Where(g=>g.isStatic) 一行补)。保留 1 个 replace_objects 有独特工程价值:PrefabUtility.InstantiatePrefab + 保持 transform/parent/sibling index + Undo group,agent 用 execute_code 现拼 15+ 行且易漏 undo/sibling,是真正的高频语义化操作。配套删除私有 helper GetAxisValue/CalculateAlignTarget/GetGameObjectPath(replace_objects 只用 ResolveGameObjects)。schema 属性字段 15→3(action/names/prefab_path,required 三项)。default case 兜底提示改为主动列出已删的 6 个 action + 各自替代方案(execute_code:run C# 片段或 find_gameobjects 工具)。文件 585 行→189 行(-396 行/-18276 字符/-68%)。栈式括号 depth=0;.cs 源码 8 个删除关键词全项目残留 0。工具总数仍 51,v1.7.21~v1.7.25 累计精简工具内 action 20 项(sort_children+arrange_grid 2 + workflow 12 + smart_operations 6)+ 内部 helper 12 项。**v1.7.21~v1.7.25 五连弹收官**:通用能力优先原则从 agent 引导层(SOUL §2 第 10 条)+能力层(execute_code:run 的 RunUsings 扩至 10 命名空间 + SerializedProperty 嵌套 propertyPath 可见性)+存量代码层(3 个工具的特化 action 大精简)三管齐下,是 v1.7.x 系列最大规模的原则一致性冲刺。；1.7.24 (2026-07-21) — v1.7.24: 工具通用能力升级第 4 弹(workflow 大瘦身)。WorkflowTool 从 15 action 精简到 3。删除的 12 个 action(batch_set_tag/layer/active/static、collect_by_component/tag/layer、batch_add_component/remove_component/move_to_parent、count_objects、list_scenes)全部可用 execute_code:run 一段 3-8 行 C# 完整覆盖(LINQ + FindObjectsByType + foreach 赋值/AddComponent/SetParent),无独特语义价值。保留 3 个 action 各有 execute_code:run 无法优雅表达的独特价值:batch_rename(占位符 pattern 语义:{index}/{name}/{parent}/{index:00} 编号 + 原名 + 父名替换)、find_replace_name(场景全量遍历 + 正则/文本 find-replace + dry_run 预览)、snapshot_hierarchy(递归深度限制 + 结构化 JSON 树输出便 agent 一次消化整场景结构)。配套删除私有 helper: ResolveLayer/IsValidTag/ResolveStaticFlags(及其 #pragma warning disable/restore CS0618 legacy NavMesh 兼容包裹)。schema 属性字段从 14 精简到 10(删 tag/layer/active/static_flags/component_type/parent_name 六个仅供已删 action 用的字段)。default case 兜底提示改为主动告知 agent 已删除的 12 个 action 名单 + 应改用 execute_code:run 替代。文件大小: WorkflowTool.cs 1211 行→467 行(-744 行/-29424 字符)。语法平衡验证:栈式括号计数(含 verbatim string/comment 排除)最终 depth=0;5 个保留 helper(GetTargetGameObjects/GetAllGameObjects/CollectAllChildren/BuildHierarchyNode/GetGameObjectPath)引用全部命中定义,无 dangling 调用。工具总数仍 51(未删工具本体,只精简工具内 action)。本弹是"通用能力优先于特化堆砌"原则的最大规模落地(单文件 -60% 代码量)。；1.7.23 (2026-07-21) — v1.7.23: 工具通用能力升级第 3 弹（正面回应用户对特化工具堆砌的持久质疑）。manage_gameobject 删除两个纯几何/纯排序特化 action:sort_children + arrange_grid。sort_children(v1.7.16 附加): HandleSortChildren 主方法(L459-507)+SortChildrenRecursive 辅方法(L514-542)+XML doc(L453-458)+schema order/recursive 两个专用参数+tool Description 一处引用+dispatcher case 一处+Unknown action 兜底提示一处,共约 91 行删除。arrange_grid: HandleArrangeGrid handler(L813-858)+schema columns/spacing/start_position 三个专用参数+tool Description 一处引用+dispatcher case 一处+兜底提示一处,共约 47 行删除。两者均可完全用 execute_code:run 一段 5-15 行 C# 覆盖(LINQ OrderBy+SetSiblingIndex 排序、foreach 赋 transform.position 网格),无独特语义价值。manage_gameobject actions 从 13 精简到 11;工具总数仍 51(没删工具,只精简工具内特化 action)。全项目 grep 交叉确认 sort_children/SortChildren/HandleSortChildren/arrange_grid/ArrangeGrid/HandleArrangeGrid 六关键词残留 0(源码+资源+plan 目录之外全扫)。文件大小: ManageGameObjectTool.cs 44526→36373(-8153 字符)。静态验证:括号平衡 205/205。本弹佐证"通用能力优先于特化堆砌"原则可落地删存量代码,不只是新增引导。；1.7.22 (2026-07-21) — v1.7.22: 工具通用能力升级第 2 弹（manage_component 嵌套 property 可见性修复）。上一版补 SOUL 引导后自查发现另一处根因盲点：ManageComponentTool.SerializeComponentDetailed 用 iterator.NextVisible(false) 只列顶层 SerializedProperty 且 key 用 iterator.name(叶名) 而非 iterator.propertyPath(完整点路径)——agent 从 get 输出里看到的字段列表根本不含嵌套字段,自然不会尝试用 stats.attack 这类 path 传给 modify。Unity 官方 SerializedObject.FindProperty 本已支持点分隔路径,缺的是 get 层的可见性。修复:(1) NextVisible(false)→true 递归进入结构/嵌套;(2) key 从 iterator.name 改为 iterator.propertyPath,输出 stats.attack、clips.Array.data[0].name 这类可直接回传的完整路径;(3) 跳过合成的顶层 Array/size 包装名(它们的 leaf 已用 Array.data[N] 露出真身,重复列纯噪声);(4) serializedProperties JObject 附 _hint 说明 key 是 path notation;(5) modify 走 FindProperty(path) 未命中的错误信息附提示引导 agent 先 get 看真实 path 键。改动文件:ManageComponentTool.cs SerializeComponentDetailed(L1146-1173) + HandleModify 错误信息(L753-760)。静态验证:括号平衡 305/305 + iterator.propertyPath 引用 1 处 + NextVisible(true) 出现 2 处 + NextVisible(false) 残留 0; 1.7.21 (2026-07-21) — v1.7.21: 工具通用能力升级第 1 弹（回应用户对"是否又在做特化工具"的质疑，正面回答"不加特化，改升级通用能力"——本弹治理 agent 撞"execute_code 只支持单表达式"墙的根因）。(1) SOUL §2.10 新增"临时代码优先 execute_code:run"引导条款：明确一次性批量/查询/自动化直接走 run 不要创建 .cs 脚本（.cs 文件会触发 Domain Reload、需菜单注册、污染项目），仅当逻辑长期复用或需 EditorWindow/MenuItem 特性时才用 manage_script；条款末尾列出可用命名空间清单。(2) ExecuteCodeTool.RunUsings 扩展导入 5 个高频命名空间（System.IO / System.Text / System.Text.RegularExpressions / UnityEngine.SceneManagement / UnityEditor.SceneManagement），对应 ReferenceAssembly 注册所在程序集（typeof(System.IO.File).Assembly 等），从 5 个命名空间扩到 10 个。(3) run 模式失败/空返回时错误信息附 RunAvailableNamespacesHint 常量（列出可用命名空间 + __result 用法 + Mono.CSharp 语法限制如不支持 async/await/顶层 return/C# 8+ 部分特性），data 字典同步注入 hint 字段——让 agent 撞墙时能自纠而非退回旧路径（创建 .cs → 菜单未注册 → 走 evaluate 又失败）。改动文件：SOUL.md（+1 条款）+ ExecuteCodeTool.cs（RunUsings 扩展 + 5 个 ReferenceAssembly 调用 + RunAvailableNamespacesHint 常量 + 2 处错误分支拼接）。静态验证：括号平衡（110/110）+ 5 处 RunAvailableNamespacesHint 引用（1 常量 + 2 data 赋值 + 2 message 拼接）+ 旧 RunUsings 字符串 0 残留；1.7.20 (2026-07-21) — v1.7.20: 设置界面本地化清理 + 两处 UI 打磨。(1) 设置界面全英文化：完整扫描 Config/Settings 相关文件，清理面向用户显示的残留中文——Log Verbosity 卡片描述、Log Level 下拉 tooltip(DashboardSettingsPage)、AgentCoreSettings.logLevel 字段 [Tooltip] 特性，共 3 处统一翻为英文；用 Python 精扫非注释行 + 兜底全扫两种方法交叉确认归零，代码注释里的中文(不显示在界面,188 处)保留不动。(2) HelpBubble 使用技巧面板精简：移除快捷键区 Ctrl+Shift+E 导出会话 / Enter 发送消息 / Escape 取消操作三条，DesiredHeight 540→620 配合内容减少尽量免滚动(面板高度仍受当前窗口物理高度 clamp,极小窗口下仍可滚)。(3) Quick Actions 按钮宽度统一：DashboardSettingsPage 的 Quick Actions 卡片 5 个按钮此前混用 140/150 致每行不齐,统一为常量 ButtonWidth=150f(取 150 容纳最长文案 Refresh Tool Registry)；1.7.19 (2026-07-21) — v1.7.19: VCS 面板 View Diff 按钮改为呼出外部图形 diff 工具。Working Copy Status 面板的 View Diff 此前把 diff 文本经 AgentCoreLog.Info 写入 Unity Console——该输出受 Settings 日志分级开关影响(调到 Warning/Error 时被吞)且极易被 Console 其他日志淹没忽略。现改为与右键菜单 Show Working Copy Diff 一致，直接呼出对应 VCS 软件的图形 diff 功能(单选 diff 该文件，否则 diff 整个工作副本，filePath 传 null 时 TryOpenExternalDiff 用 VCS 根目录)，不再写 Console；删除旧 ShowDiffAsync 实现。目前仅维护 SVN(TortoiseProc.exe /command:diff)，Git/Perforce 图形 diff 集成不做，这两种 VCS 下按钮禁用(SupportsExternalFileTool(\"diff\") 判定)避免假入口；1.7.18 (2026-07-21) — v1.7.18: 修复输入框 Shift+Enter 触发全选 / 换行交互异常。Unity 2022.3 多行 TextField 内建 KeyboardTextEditor 通过 default action 通道处理 Shift+Enter，把它错误当成全选(连续按会全选→再按替换掉整段文本)。经 DIAG 日志实证定位：全选在按键 default action 阶段注入，发生在自定义 handler 执行之后的间隙——事件拦截(StopPropagation/StopImmediatePropagation，甚至下沉阶段注册到内部 unity-text-input 子元素)全都拦不住 default action，无法靠拦截根治。解法：不再与内建抢 Enter/Shift+Enter，换行键位改用内建不响应的 Ctrl+Enter 组合键自己插入换行，彻底绕开；Enter 保持发送并保留 IME 守卫(组字未提交时 Enter 是确认选词/上屏，不发送，让候选内容上屏输入框；组字提交后再 Enter 才发送)；换行插入用 SetValueWithoutNotify + 光标钉位(立即+延迟一帧两次)避免 value setter/Focus 引发意外全选；HelpBubble 快捷键提示同步。诊断过程严守\"先完整查根因再改、不缝缝补补\"，三次试错(加 Shift+Enter case→TrickleDown→子元素注册)均经 DIAG 实证证伪拦截路线后回滚到干净基线重做；1.7.17 (2026-07-21) — v1.7.17: 修复 GameObject 工具链无法操作 inactive 对象。ToolHelpers.FindGameObject / FindGameObjectsByName 及 SmartOperationsTool 的场景对象按名查找用 FindObjectsByType<GameObject>(FindObjectsSortMode.None)，该重载在 Unity 2022.3 默认 FindObjectsInactive.Exclude，导致 inactive 对象查不到；后果是用 set_active_batch 等工具禁用一个对象后，再也无法用同一工具链(get_info/modify/set_active_batch/manage_component/smart_operations)重新找到并启用它——启用能力闭环缺口。三处查找统一补 FindObjectsInactive.Include；对照组 FindGameObjectsTool(activeOnly:false 时已正确传 Include)不受影响；SceneAnalysisTool/OptimizationTool 按 Renderer/Light 统计渲染对象保持默认 Exclude(统计活跃渲染物是正确语义,未改)。经 set_active_batch 修复(v1.7.16)的完整测试(D1~D5 全 PASS)暴露此隐患；1.7.16 (2026-07-21) — v1.7.16: 工具确认流程重构 + set_selection 增强 + set_active_batch 修复 + VCS 体验改造。(1) 确认流程重构：确认面板"弹不弹"从单一破坏性标志升级为 ToolRiskLevel（风险分级）× ToolCapability（能力位）联合决策 + action token 兜底，修复多 action 混合工具能力位"连坐"——同工具内只读 action（如 manage_gameobject:get_info）不再因该工具含破坏性 action 被误判需确认，粒度落到 action 级；涉及 ToolRiskPolicy/AgentToolAttribute/IAgentTool/ChatWindow.Confirmation + 全部原生工具 action 白名单标注。(2) set_selection 多选：manage_editor:set_selection 支持多目标/InstanceID/同名全选，兼容数组·逗号字符串·字符串化 JSON 三形态，新增 ToolHelpers.FindGameObjectsByName。(3) set_active_batch per-item active 修复：传 items:[{target,active}] 时 handler 只读顶层 active→null→回退 true 致 active:false 对象未禁用（静默给错结果）；改逐 item 读 active（缺省回退顶层再回退 true），summary 改计数式；全量排查其余 24 batch handler 无同类隐患。(4) VCS 体验改造：删除 SceneView 顶部黄色警示条 VcsSceneViewUpdateBanner（+6 处 DisplayDialog）；VCS 页面 Update 去系统确认框直开外部窗口；新增通用 HubNavBadgeBus 角标总线 + HubRail.SetModuleLabel/SetModuleAlert，VCS 侧 VcsHubNavBadgeDriver 按检测类型把左侧导航按钮标签改为 SVN/GIT/P4 且远端有更新时变黄；VCS asmdef 单向依赖主 Editor 经总线通信无循环依赖；1.7.15 (2026-07-20) — v1.7.15: 修复工具确认面板不弹（信任 scope 无感知残留）：破坏性工具（如 manage_gameobject:create）被策略引擎正确判为 RequireConfirmation，却直接执行不弹面板。根因是会话级信任 scope（YOLO / Trust Low-Med）残留——用户先前点过 YOLO/Trust，经 SessionState 持久化后新建对话 session 时未清除，导致 IsToolConfirmationTrusted 直接返回 true 跳过面板；策略判定链（ToolRiskPolicy → ExtractActionFromParameters → RequiresDestructiveActionConfirmation）全程正确，缺陷仅在信任生命周期。修复将工具信任生命周期绑定到"对话 session"：OnNewSessionClicked + SwitchToSession 两处切换对话入口新增 ClearPendingToolConfirmations()，新建/切换对话即失效；Domain Reload（脚本重编译）仍保留信任（SessionState 不变），Editor 完全重启自然归零。诊断经 DIAG-POLICY 临时日志实证定位后移除；1.7.14 (2026-07-20) — v1.7.14: ask_user 中途提问工具（挂起-唤醒完整态，复刻 WaitingCompilation 范式，完全不碰 SelfChallenge）：Agent 遇实现方向岔路（多方案权衡/需求歧义/继续须假设）时主动调 ask_user 提问，独立选项面板阻断（无超时·不自动拒绝·永久等待），loop 检测 ToolResult.IsAwaitingUserInput 标志→SetState(WaitingForUserInput)→截断退出循环不空等；用户点选项或「我自己描述」自由文本应答后，ResumeFromUserInput 追加 user 消息 + TriggerResumeLLMCall 唤醒 LLM 继续；挂起状态存 DomainReloadState（3 字段）跨 domain reload 存活，reload 后 TryRestorePendingAskUser 重建面板。新增 AgentLoop.AskUser.cs + ChatWindow.AskUser.cs + AskUserTool.cs（纯函数化，自动发现注册）；ToolResult 加 IsAwaitingUserInput/AskUserQuestion/AskUserOptions 字段。时序竞态经 delayCall 排除。SOUL §2 加中途收束方向引导；1.7.13 (2026-07-20) — v1.7.13: 会话自动重命名（右键菜单基于最近 12 条上下文调 LLM 生成反映当前主话题的标题，SessionAutoTitleService）+ 助手气泡宽度修复（AssistantTurnView 6 个 wrapper 设 width:100%，消除首段短内容压窄后续工具卡片/长内容）+ ROADMAP 同步实现现状（Tier 0/1/2 标记完成 + ADR-19）；v1.7.12 (2026-07-20) — v1.7.12: 日志分级全量贯彻（83 文件 329 处裸 Debug.* → AgentCoreLog.*：Warning 245 / Error 74 / Info 10，补 62 文件 using；此前绕过 Settings Log Verbosity 开关永远输出，现全部日志受分级控制）；v1.7.11 (2026-07-18) 三项修复（HubRail 仅保留 Settings 齿轮图标其余回退文字，消除图标/文字割裂 / StreamingTextElement SanitizeForDisplay 清洗无法渲染字符消除对话 □ 方块 / ManageComponentTool 支持 ObjectReference 字段赋值 instanceId·资源路径·场景名·null + GameObject↔Component 转换）；v1.7.10 (2026-07-18) UI 收尾（MessageReferenceBar chip 截断修复 minHeight+内容撑高 / HelpBubble 补齐输入框快捷键 Enter·Shift+Enter·Ctrl+N·Escape / HubRail 图标化 ResolveBuiltinIconName+TryApplyIcon 带文字回退 / MessageListManager.LoadMoreItems 滚动位置补偿消除加载更多跳动；P3 项 ScrollToBottom 双帧延迟·error stretch·键盘可访问性经核实判定无需改动）；v1.7.9 (2026-07-17) — v1.7.9: HelpBubble 浮窗根因修复（ResolveMountRoot 挂载点）+ 对话泡泡引用溢出/窗口缩窄卡死修复（GeometryChangedEvent 反馈死循环三重防护 + MinWindowSize 420）+ UI 交互视觉审查 P0/P1（IME Enter 误发送守卫 / GetContextBudget 移出高频路径 / 流式视觉跳变方案C 统一 block 渲染 / ToolCallCard 超长结果 8000 字符截断显示 / 色板统一 AgentCoreColors 单一真源）；v1.7.7~1.7.8: 修复 Preferences 目录 Move 弹窗根因（_cachedDirEnsured 缓存 + 目录扫描选错版本 + SafeSave 返回值）+ CS0103 打包遗漏修正；v1.7.6: 修复 v1.7.5 遗漏的 using 指令（CS0103）；v1.7.5: Preferences 路径硬编码兜底 + ScriptingDefineSymbols 版本兼容（Unity 2023+/6 ForGroup API 废弃迁移）；v1.7.4: 修复路径解析根因（Unity 内部版本号 vs 营销版本号不匹配，三级路径解析）；v1.7.3: beforeAssemblyReload 回调；v1.7.2: minimalism / self-challenge 历史文档整理归档；v1.7.1: 修复新装路径不存在（[InitializeOnLoad] 静态构造函数）+ VCS 远端检查开机触发（_lastCheckedUtc 初始化为 UtcNow）+ 3 个 CS0162 编译警告清理 + SessionStorage 日志降级；v1.7.0: Settings v20 死字段清理（12 字段删除 + disabledTools 默认值修正 + Model Info 显示 effective tokens）+ VCS 模块修复（进程句柄泄漏 / 事件泄漏 / Debug.Log 风暴 / 设置极简化 8→2 / Project 窗口多 VCS 支持 / Perforce ignore 标注） |
...
### 3.w.1 v1.8.0 收尾（2026-07-23）— **已完成**

**A×B 双轴重扫结果**：44 项候选缩到 **28 项真缺口**（P0=7 / P1=11 / P2=10）。用户决策路径 A：v1.8.0 一次性做完全部 P0 7 项 + 2 个副产物 Bug hotfix。

**已实施 P0 清单（全部编译验证 + 运行时验证）**:

| 项 | 缺口类型 | 落地 | 验证 |
|---|---|---|---|
| G27 | DISCOVERABILITY | `manage_profiler` Visibility=AlwaysVisible + SOUL §2.11 Profiler use hints 引导 | 编译+运行时 ✓ |
| G01 | SHALLOW | `manage_profiler` 新增 `list_available_stats` / `sample_recorder` | 编译+运行时（真实数据）✓ |
| G02 | NO_TOOL | `manage_profiler` 新增 `get_frame_range` / `read_frame`（HierarchyFrameDataView + 多线程） | 编译+运行时（219 帧 buffer, EditorLoop 228ms/4.4FPS）✓ |
| G03 | NO_TOOL | `manage_profiler` 新增 `list_draw_events` / `get_draw_event` / `disable_frame_debugger`（反射 `UnityEditorInternal.FrameDebuggerInternal.FrameDebuggerUtility`） | 编译+反射链路验证（SetEnabled 成功、count 返回、结构化 hint 完整；实际 event 采集受环境 blocker 未真实回头）✓ |
| G10 | NO_TOOL | `manage_graphics` 新增 `volume_list` / `volume_get` / `volume_set`（反射 URP/HDRP Volume；`AGENTCORE_HAS_SRP_CORE` Version Defines 隔离 SRP 依赖） | 编译+运行时（built-in fallback stub 通过；SRP 环境留后续）✓ |
| G17 | NO_TOOL | `manage_asset` 新增 `find_references`（AssetDatabase.GetDependencies 反向扫描 + filter + recursive） | 编译+运行时（89 asset 301ms / filter 5 asset 1ms / recursive 243ms）✓ |
| G18 | NO_TOOL | `scene_analysis` 新增 `find_references_in_scene`（跨 asset↔scene SerializedObject walker + sub-asset） | 编译+运行时（`mat_curb.mat → Road/RoadBuilder.curbMaterial` 精准命中自定义脚本字段）✓ |

**副产物 Bug hotfix**:
- Bug#1: `manage_camera action=render_to_texture` — ImageConversion 引用修复 ✓
- Bug#2: `execute_code` — Texture2D.EncodeToPNG 引用修复 ✓

**反射盲写 pitfall 修复**: G02 隐藏依赖 `ProfilerDriver.enabled` vs `Profiler.enabled` 双开关，此前只开 `Profiler.enabled` 导致 `read_frame` 一直返 0 帧；同步开 + `ProfilerDriver.profileEditor=true` + hint 分层诊断。

**发版期间发现的性能 issue**: agent 流式输出阻塞 Editor 主线程（228ms/帧 = 4.4 FPS），详见 `plans/perf-issue-agent-streaming-blocks-editor.md`，v1.8.0 后续版本优化，不阻塞本次发版。

**技能新增 references**: `references/agentcore-execute-code-constraints.md`（Mono.CSharp.Evaluator 硬约束 — 无 using 指令、无 top-level return、无 C# 8+ 语法、`Object` 歧义、Scripting 类别 activation gate、反射探测两阶段模式）。

**Deferred（原锚点 v1.9.0+，2026-08-11 现状核验：仍未实施，版本已远超锚点）**:
- P1 11 项 + P2 10 项能力覆盖缺口（后续按需重新评估，无强制时间表）
- MCP-over-tools 集成（Phase 8，见 §3.x —— 2026-08 起前置条件已满足，待用户决定是否重启）
- Agent 流式输出主线程优化（v1.8.0 发版时发现 228ms/帧，随后 v1.12.0-alpha.4 的 `EndOfStream` 阻塞根因修复 + v1.14.x 双阈值 flush/UI 帧节流已显著缓解，未做专项归档）

---

## 3.x Phase 8 — MCP 对外互操作（对外 / 曾 DEFERRED，2026-08 起**条件已满足待重启**）

> **⚠ 历史**: v1.8.0 范围变更 (2026-07-22) 时用户明确"MCP拓展不是 1.8.0 的内容"，Phase 8 从 v1.8.0 顺延，DEFERRED 至 v1.9.0+。**2026-08-11 (v1.14.13) 更新**：原 deferred 的两个前置条件现已全部满足——(1) 版本已远超 v1.9.0（当前 v1.14.13）；(2) Phase 10（§3.w 能力覆盖面补齐）的 P0 已全部完成（其剩余 P1+P2 与 MCP 无前置依赖）。故 Phase 8 具备重启实施条件，具体是否/何时启动由用户决定。历史内容以下保留作为设计沉淀。
...
v1.7.4        — 修复 Preferences 目录路径解析根因（Unity 内部版本号 vs 营销版本号不匹配，三级路径解析）
v1.7.5        — Preferences 路径硬编码兜底 + ScriptingDefineSymbols 版本兼容（Unity 2023+/6 ForGroup API 废弃迁移）
...
| [`indexing-background-incremental-design.md`](_archive/features/indexing-background-incremental-design.md) | **Phase 7 §3.1** 后台静默 + 增量索引详细设计（v1.1.0 上游依据，已归档） | `_archive/features/` |
| [`indexing-scope-layered-and-status-awareness-design.md`](_archive/features/indexing-scope-layered-and-status-awareness-design.md) | **Phase 7 §3.1.1** Scope 层次化索引 + LLM 状态感知详细设计（v1.4.0 上游依据，已归档） | `_archive/features/` |
...
> 当前基线已推进到 **v1.14.13 (2026-08-11)**。自演化知识系统 Tier 0/1/2（Memory / Init Project / Skills+Curator）已落地——其中 Tier 1 由「静态拆分文档 + 手动 re-init」演进为「每次 Bootstrap 实时静默收集 + 缓存注入」，效果更优（不过期、零手动），见 ADR-19。其余方向均为 P2（未启动），按优先级排列如下。
...
| ✅ 已完成 | **Session 搜索 / Session 分支（原"已排除"项，2026-08 更新）** | 两者均已以 v1.14.10 `search_sessions` 跨会话全文检索工具、v1.14.8 Fork 会话（从历史节点复制新会话）形式落地实现，故从"已排除方向"移除 |
| P2 | **Goal 模式** | 跨轮持久化目标，agent 每轮记得当前大任务；session 级不跨 session；每轮注入 system prompt；agent 回复末尾附 goal 进度一行 |
| P2 | **MCP Server — 协议骨架（8.1.1 ~ 8.1.4）** | JSON-RPC + stdio，对外互操作；治理前置 G.1/G.2/G.3 已满足。**2026-08-11 更新：deferred 条件已满足（见 §3.x），待重启排期** |
| P2 | **MCP Server — 传输与兼容性（8.1.5 ~ 8.1.7）** | stdio 稳定后扩展 HTTP/SSE 传输；覆盖 Claude Desktop / Cursor / Continue / CLI 四类客户端 |
| P2 | **视觉感知辅助诊断（Vision-Assisted Diagnostics）** | 让 agent 能"看到" Game View / Scene View 画面，辅助诊断依赖视觉表现的问题（穿模、材质颜色、布局错位）。可行性待模型 vision 条件重新评估（2026-08-11 更新：主模型已由 GLM-5.2 切至 DeepSeek-V4-Flash，需重新确认当前部署是否支持 image input；此前 GLM-5.2-W4AFP8 不支持 image input，代理层剥离图片并注入 reminder）。技术路径：Game View 用 `ScreenCapture.CaptureScreenshotAsTexture`，Scene View 用 `SceneView.lastActiveSceneView.camera` → RenderTexture；两级调用（截图 → 辅助 vision 模型转文字描述 → 注入主对话）或等主模型支持 vision 后直接注入 |
| P2 | **Phase 7 §3.4 产品化 — UPM 发布流程（7.4.1）** | v1.7.2 已是稳定产品，发布流程可沉淀为自动化脚本 |
